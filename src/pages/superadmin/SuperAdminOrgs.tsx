...
import { Search, Building2, Users, ChevronDown, UserCheck, Sparkles, Trash2, ExternalLink } from 'lucide-react'
...
type OrgTab = 'organisations' | 'utilisateurs'
...
// ── Mes organisations section ─────────────────────────────────

function MesOrganisations() {
  ...
  async function handleAcceder(orgId: string) {
    try {
      ...
      // If ensure-org-access fails, still try to navigate
    }
    setOrgContext(orgId)
    ...
    <div className="mb-8">
      ...
      <div className="flex flex-wrap gap-2">
        {orgs.map(o => (
          <button
            key={o.org_id}
            onClick={() => handleAcceder(o.org_id)}
            disabled={ensureAccess.isPending}
            className="flex items-center gap-2 px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-sm text-slate-300 hover:text-white hover:border-brand-500 hover:bg-slate-700 transition-colors disabled:opacity-50"
          >
...
export default function SuperAdminOrgs() {
  ...
  const activeTab: OrgTab = searchParams.get('tab') === 'utilisateurs' ? 'utilisateurs' : 'organisations'
  ...
  return (
    <motion.div initial={{ y: 8, opacity: 0 }} animate={{ y: 0, opacity: 1 }}>
      ...
      {/* Sub-tabs */}
      <div className="flex items-center gap-0 border-b border-slate-700 mb-6">
        {tabs.map(tab => (
          <button
            key={tab.id}
      ...
      {/* Tab content */}
      {activeTab === 'organisations' && <OrganisationsList />}
      {activeTab === 'utilisateurs'  && <SuperAdminUsers />}
    </motion.div>
  )
}
