...
### [BUG-002] admin_audit_log : INSERT bloqué pour les non-superadmins
- **Statut** : resolved
- **Priorité** : critical
- **Résolu en** : migration 20260507004_audit_fixes.sql
- **Description** : La policy RLS `FOR ALL USING (is_superadmin())` bloquait les INSERTs des utilisateurs non-superadmin. Les logs d'audit Stripe (appelés depuis `stripe.service.ts` via `logEvent()`) étaient silencieusement abandonnés.
- **Note** : Scission en `audit_log_superadmin_read` (SELECT) + `audit_log_authenticated_insert` (INSERT).
...
- **Priorité** : critical
- **Résolu en** : migration 20260507004_audit_fixes.sql (policy INSERT)
...
- **Statut** : resolved
- **Priorité** : critical
- **Résolu en** : migration 20260507004_audit_fixes.sql (fix structurel), workaround client conservé
